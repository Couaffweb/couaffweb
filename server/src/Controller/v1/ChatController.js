...
class ChatController extends ApiController {
	async sendMessage(Request) {
		const required = {
			...
			videoLength: Request.body.videoLength || '0'
		};
		...
		const user_info = await DB.find('users', 'first', {
			...
			fields: ['id', 'name', 'status', 'email', 'phone', 'profile', 'status'],
		});
		...
		if (user_info.profile) {
			user_info.profile = global.appURL + 'uploads/' + user_info.profile;
		}
		...
		setTimeout(() => {
			const pushObject = {
				message: parseInt(required.message_type) === 0 ? requestData.message: 'sent a media file',
				notification_code: 8,
				body: requestData,
			};
	...
	async getMessage(Request) {
		...
		const requestData = await super.vaildation(required, {});
		const { user_id, friend_id } = requestData;
		const thread = `select threads.*,threads.id as thread_id from threads where (threads.user_id = ${user_id} and friend_id = ${friend_id}) or (threads.user_id = ${friend_id} and friend_id = ${user_id}) limit 1`;
		let id = 0;
		...
		const query = `select chats.*, users.id as friend_id, users.profile, users.name, users.profile, users.phone,users.email from chats join users on (users.id = IF(chats.sender_id = 
			${user_id},chats.receiver_id,chats.sender_id)) where thread_id=${threadInfo[0].thread_id} and chats.id > ${id} and (select count(id) as total from delete_chats where user_id =  ${user_id} and chat_id = chats.id) = 0 limit 100`;
	...
	async lastChat(Request) {
		...
		const query = `select users.id as friend_id,(select count(id) from chats  where is_read = 0 and receiver_id = ${user_id} and sender_id = users.id) as un_read_message,users.name, users.profile, users.phone,users.email,
		chats.*
		from threads join chats on (chats.id = threads.last_chat_id) join users on (users.id = IF(user_id = ${user_id}, friend_id, user_id )) 
		where (threads.user_id = ${user_id} or  threads.friend_id = ${user_id}) and chats.id > IF(threads.user_id = ${user_id}, threads.first_friend_deleted_id, threads.second_friend_deleted_id)  order by chats.id desc`;
		return {
...
const makeChatArray = (chats, spaceInfo = {}) => {
	return chats.map((value) => {
		const chats = {
			id: value.id,
			sender_id: value.sender_id,
			videoLength: value.videoLength || '0',
